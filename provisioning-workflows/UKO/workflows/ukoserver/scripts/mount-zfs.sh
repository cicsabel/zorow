...
if [ ! -d "${instance-WLP_USER_DIR}/servers/${instance-UKO_SERVER_STC_NAME}" ]; then
    echo "Creating server directory ${instance-WLP_USER_DIR}/servers/${instance-UKO_SERVER_STC_NAME}"
    mkdir -p ${instance-WLP_USER_DIR}/servers/${instance-UKO_SERVER_STC_NAME}
    if [ $? -gt 0 ]; then 
...
if [ -z "${mountvalue}" ]; then
    echo "Running command: mount -t ZFS  -f ${zfs} ${instance-WLP_USER_DIR}/servers/${instance-UKO_SERVER_STC_NAME}"
    mount -t ZFS -f ${zfs} ${instance-WLP_USER_DIR}/servers/${instance-UKO_SERVER_STC_NAME}
else
...
if [ ! -d "${instance-WLP_OUTPUT_DIR}/${instance-UKO_SERVER_STC_NAME}" ]; then
    echo "Creating output directory ${instance-WLP_OUTPUT_DIR}/${instance-UKO_SERVER_STC_NAME}"
    mkdir -p ${instance-WLP_OUTPUT_DIR}/${instance-UKO_SERVER_STC_NAME}
    if [ $? -gt 0 ]; then 
        echo "ERROR: Could not create directory" >&amp;2;
        exit "2"; 
    fi
...
if [ -z "${mountvalue}" ]; then
    echo "Running command: mount -t ZFS e -f ${zfs} ${instance-WLP_OUTPUT_DIR}/${instance-UKO_SERVER_STC_NAME}"
    mount -t ZFS -f ${zfs} ${instance-WLP_OUTPUT_DIR}/${instance-UKO_SERVER_STC_NAME}
else
