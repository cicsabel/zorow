...
sh
/usr/sbin/unmount -o immediate /u/${instance-SERVER_STC_USER};
if [ $? -gt 0 ]; then 
    echo "no web task user directory needed to be unmounted";
fi;
/usr/sbin/unmount -o immediate /u/${instance-AGENT_STC_USER};
if [ $? -gt 0 ]; then 
...
//SYSIN    DD *,SYMBOLS=JCLONLY
 DELETE OMVS.${_workflow-systemName}.USERID.${instance-SERVER_STC_USER}
 IF MAXCC EQ 8 THEN DO
   SET MAXCC = 0
   END
 DELETE OMVS.${_workflow-systemName}.USERID.${instance-AGENT_STC_USER}
 IF MAXCC EQ 8 THEN DO
   SET MAXCC = 0
   END
/*
...
SH
rm -rf /u/${instance-SERVER_STC_USER};
if [ $? -gt 0 ]; then 
    echo "no web task user directory needed to be deleted";
fi;
rm -rf /u/${instance-AGENT_STC_USER};
if [ $? -gt 0 ]; then 
